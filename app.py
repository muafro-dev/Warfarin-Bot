...
# --- LANGCHAIN (Only for Vector Store) ---
...
try:
    ...
    # Configure the NATIVE client directly (The Real Fix)
...
if prompt := st.chat_input("Ask about Warfarin protocols..."):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            # 1. Retrieve Docs using LangChain (Vector Store)
            ...
            # 2. Generate Content using NATIVE CLIENT (Bypasses LangChain 404 errors)
            ...
            # THE SAFETY AIRBAG: Try/Except block like the other AI
            try:
                response_obj = model.generate_content(full_prompt)
                response_text = response_obj.text
            except Exception as api_error:
                # If API fails, fall back to a safe error message instead of crashing
                response_text = f"⚠️ AI Service Unavailable. Here is the top retrieved text snippet:\n\n{docs[0].page_content}"
